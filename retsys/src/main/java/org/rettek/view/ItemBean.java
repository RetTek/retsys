...
public class ItemBean implements Serializable
{
   ...
   private Predicate[] getSearchPredicates(Root<Item> root)
   {
      ...
      String brand = this.example.getBrand();
      if (brand != null && !"".equals(brand))
      {
         predicatesList.add(builder.like(builder.lower(root.<String> get("brand")), '%' + brand.toLowerCase() + '%'));
      }
      String color = this.example.getColor();
      if (color != null && !"".equals(color))
      {
         predicatesList.add(builder.like(builder.lower(root.<String> get("color")), '%' + color.toLowerCase() + '%'));
      }
      String unit = this.example.getUnit();
      if (unit != null && !"".equals(unit))
      {
         predicatesList.add(builder.like(builder.lower(root.<String> get("unit")), '%' + unit.toLowerCase() + '%'));
      }
      String size = this.example.getSize();
      if (size != null && !"".equals(size))
      {
         predicatesList.add(builder.like(builder.lower(root.<String> get("size")), '%' + size.toLowerCase() + '%'));
      }
