...
public class PurchaseOrderBean implements Serializable
{
   ...
   private Predicate[] getSearchPredicates(Root<PurchaseOrder> root)
   {
      ...
      String deliveryAddress = this.example.getDeliveryAddress();
      if (deliveryAddress != null && !"".equals(deliveryAddress))
      {
         predicatesList.add(builder.like(builder.lower(root.<String> get("deliveryAddress")), '%' + deliveryAddress.toLowerCase() + '%'));
      }
